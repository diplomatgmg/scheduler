...
@router.callback_query(PostScheduleCallback.filter(F.action == PostScheduleEnum.SET_DELETE_TIMER))
async def handle_set_delete_timer(query: CallbackQuery) -> None:
    """Выкладывает пост в определенное время."""
    logger.debug(f"Handle set delete timer callback from {get_username(query)}")
